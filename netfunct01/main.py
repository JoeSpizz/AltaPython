...
def commandpush(devicecmd): # devicecmd==dict
    ...
    for ip in devicecmd.keys(): # looping through the dict
        print(crayons.red(f'Handshaking. .. ... connecting with {ip}')) # fstring
        # we'll learn to write code that connects to devices here
        for mycmds in devicecmd[ip]:
            print(crayons.green(f'Attempting to sending command --> {mycmds}'))
            # we'll learn to write code that sends cmds to device here
    return None

def devicereboot(devicecmd): 
    for ip in devicecmd.keys():
        print(crayons.yellow("Connecting to..." + ip))
        print(crayons.green("REBOOTING NOW"))
    return None
...
def main():
    ...
    devicereboot(devicecmd) #call function to reboot devices
